...
class design_request(models.Model):
    ...
    def action_ready_for_quotation(self):
        for record in self:
            ...
            if record.state != "in_progress":
                raise UserError(
                    "Cannot move to 'Ready for Quotation' unless the record is 'In Progress' or state is "
    ...
    def action_generate_quotation(self):
        for record in self:
            if record.state != "ready_for_quotation":
                raise UserError(
                    "You cannot generate a quotation until the state is 'Ready for Quotation or once the "
            ...
            product = self.env["product.product"].search(
                [("name", "=", record.design_name)], limit=1
            )
            if not product:
            ...
            quotation.message_post(
                body="Sales Quotation has been successfully generated for the design request.",
    ...
    def action_send_for_client_review(self):
        for record in self:
            if record.state != "generate_quotation":
                raise UserError(
                    "Cannot send for client review until a quotation is generated."
                )
    ...
    def action_cancel(self):
        for record in self:
            if record.state in ["generate_quotation", "send_for_client_review"]:
                raise UserError(
                    "Cannot move to 'Cancelled' once a quotation is generated or sent for client review."
                )
            record.state = "cancelled"

    def action_send_mail(self):
        template = self.env.ref(
            "design_request.design_request_in_progress_email_template"
        )
        if not template:
        ...
        for record in self:
            if record.customer_email:
                try:
                    ...
                    _logger.error(
                        "Failed to send email to %s: %s", record.customer_email, str(e)
                    )
            else:
